...
public class MSCognitiveServicesClassifier {
    ...
    public MSCognitiveServicesClassifier(final Context context, String path, String labelPath) {
        inferenceInterface = new TensorFlowInferenceInterface(context.getAssets(), path);
    ...
    private void loadLabels(final Context context) {
        ...
        try {
            final InputStream inputStream = assetManager.open(labelPath);
    ...
    public Classifier.Recognition classifyImage(Bitmap sourceImage, int orientation) {
        ...
        if (maxConf < 0.2){
            return new Classifier.Recognition("0", null, maxConf, null);
        }
